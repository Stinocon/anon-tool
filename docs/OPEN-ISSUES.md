...
State at 2026-09-23 (last reviewed): everything closed so far carries its evidence next to it. **What
is genuinely open is short**: the second half of #26 (the scan locator is still one alternation over
every first token — measured at 0.39 MB/s with the catalogs, harmless for the guard which does not load
them), and #40, which is the operator's step rather than the code's. #13 is withdrawn and #19 needs no
action, both on purpose.
