...
| 5 | 17 | **Optional local-model detector** (DESIGN §7): a localhost endpoint that *suggests* candidates which a human approves — **the seam AND the approval step exist** (`suggest.py`: loopback-only, fail-closed, writes nothing, engine never imports it, `--api-key`/`--header` for the local server, `reasoning_content` fallback; the UI panel in the Dizionario tab with `--suggest-url/--suggest-model`, always visible and inert without them; DEC-0018, DEC-0019). What is LEFT is the model: measured with the local MTPLX Qwen3.5-9B, a one-sentence text gets a correct proposal in ~30 s while a richer one spends 138 s and 4096 tokens and returns nothing (`finish_reason: length` — a reasoning model that thinks until the budget ends). The seam reported it as an error, correctly. Next: a smaller instruct model, or a larger `--max-tokens`, or any OpenAI-compatible loopback server. | The structural answer to contextual references — the biggest declared hole. The engine stays the only writer, so determinism is untouched. | large (remaining: medium) |
...
| 21 | 38 | **The web UI in English as well as Italian.** The interface is Italian-only (`web/index.html`, `web/app.js`); the CLI, the code, the tests and the docs are English. The work is not translation but plumbing: a string table with two locales, a switch that remembers the choice, every place that asserts Italian copy (`tests/test_web.py`, `tests/ui_load_check.mjs` — the declared ids), the README screenshot, and the README line that says the UI is Italian. The ENGINE is not involved: it is already language-neutral apart from the Italian validators and the `legal` pattern group (see "Italian first, not Italian only"). | The README states it plainly, so an English-speaking reader meets an Italian interface at the first click. | medium |
