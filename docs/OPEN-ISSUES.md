...
| 5 | 17 | **Optional local-model detector** (DESIGN §7): a localhost endpoint that *suggests* candidates which a human approves. The seam, the approval panel and now the SHIPPED MODEL are done (2026-09-23): `make model` fetch script downloads Qwen2.5-3B-Instruct Q4_K_M (~2 GB) verifying size and sha256, and `docker-compose.model.yml` runs it as a sidecar (the expected size/sha256 come from the model repository's metadata API and are confirmed only when a first download completes — until then they are "published", not "verified") with `network_mode: service:anon-tool` — sharing the UI's network namespace is what keeps the seam's loopback-only rule (DEC-0018/0019) TRUE inside the container instead of relaxing it, and the port is never published. `scripts/bench-suggest.py` measures any loopback endpoint; the model-dependent tests run only when `ANON_MODEL_URL` is set, so `make test` stays green without a model. **Measured through the seam on this machine**: a 9B reasoning model answered in **43.4 s with no JSON** (a "Thinking Process" narrative) and the seam refused it as an error, never as an empty list — the reason the shipped model is a small INSTRUCT one with a bounded `--max-tokens` (512). **MEASURED 2026-09-23** (the model is downloaded; the published size and sha256 were confirmed by the first completed download, so they are verified, not merely published): `make model` then the UI's own path, 570-character Italian report, 6 CPU threads → **37.3 s end to end**, **5 proposals** (Ancona, Milano, Prato, Bologna, one address), of which **4 were not found by the engine** (it found the email and the phone). Prompt 21.6 tokens/s, generation 6.7 tokens/s: the latency is the cost, and it is why the panel is a separate step. The gated test passes against the containerized model. | The structural answer to contextual references — the biggest declared hole. The engine stays the only writer, so determinism is untouched. | medium (plumbing done; the model's numbers pending) |
...
| 17 | 26 | **Guard throughput vs dictionary size.** Half closed (2026-09-23). The scan index is now built once per run and reused, and a position is resolved through a sub-index on the SECOND token instead of probing every entry that shares a first name: the synthetic law halved (5 MB corpus, end to end — 200 entries 1.81 MB/s · 294 → 1.75 · 1 000 → 1.38 · 4 000 → 0.73 · 7 900 → 0.44), and a 10 000-entry dictionary that shares first names no longer turns the scan quadratic (the measured cost was `matches × entries sharing a first token`: 14.3M anchored probes). The equivalence test now covers the shared-first-token shape, and fails if the narrowing is wrong. **What remains is the LOCATOR**: the first-token alternation is one regex with one branch per source, and with the catalogs (329 sources, common words) it alone costs 8.8 s of 9.1 s on a 5 MB real document — 0.39 MB/s, because `re` tries every branch at every position. A synthetic corpus hides this: it inserts each name once, so the bench reads 1.75 MB/s at 294 entries where a real document with the catalogs reads 0.39. The fix is a token-indexed locator (a word scan plus dict lookups, alternation only for sources with non-word characters); it needs the same equivalence proof. Not urgent for the guard — it runs `--check` without `--catalogs` and holds 23 MB/s with the operator's 12-entry dictionary — but `--catalogs` on a large document is 30 s for 12 MB. | 12 MB / 20 s were sized on this Mac with a few hundred entries; the margin is not a property of the guard, it is a property of the dictionary AND of how often the locator's words occur in the text. | medium |
...
| 21 | 38 | **The web UI in English as well as Italian.** CLOSED 2026-09-23: the header carries a language selector, Italian is the default and the source (the text stays in `web/index.html`), English lives in `web/i18n.js` keyed by a CSS selector per element — a paragraph that mixes text with `<strong>`/`<em>`/`<code>` is replaced whole, because word order differs between languages and translating a text node would produce English that does not compose. The choice persists in `localStorage` (`anon-lang`), `applyLanguage` restores Italian from a snapshot taken at load, `app.js` keeps an Italian fallback so a missing `i18n.js` cannot break the page, and a test fails when a dictionary key no longer matches an element. 50 keys, 41 web tests. **Declared limit**: the messages the SERVER produces (progress, errors) and the tooltips are still Italian — they need a language parameter on the API, not a client-side table. | The README stated it plainly, so an English-speaking reader met an Italian interface at the first click. | medium |
| 22 | 39 | **CI: bump the action majors GitHub now annotates.** The workflow pins `actions/checkout@v4`, `actions/setup-node@v4`, `actions/setup-python@v5`; the runners force them onto Node 24 and annotate every run (green, not a failure). Bump the majors, re-run the suite, and update the run-summary line if the step names change. | An annotation printed on every public run is what a reader notices before the green check. | small |
| 20 | 37 | **The two-block rule of `catalogs/vendors.txt` is a prose convention, not an enforced invariant** — CLOSED in the sixth pass (see above): `VendorsCatalogTest` fails a name in the wrong block, in both directions, and sweeps block 2 against the OS dictionary. Two adversarial reviews of the first version moved FIFTEEN names — six ordinary words in the case-insensitive block (`gigabyte`, `red hat`, `avast`, `veritas`, `trend micro`, `western digital`), three needlessly case-sensitive (`aruba`, `kingston`, `eaton`), `cisco` put in the wrong block BY the fix itself, `okta` (a cloud-cover unit), and `ibm`/`amd`/`hpe`/`apc` kept case-sensitive on a rationale that was false, which made `un server ibm` a MISS. |  | — |
