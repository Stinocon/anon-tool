...
| 5 | 17 | **Optional local-model detector** (DESIGN §7): a localhost endpoint that *suggests* candidates which a human approves — **the seam exists** (`suggest.py`: loopback-only, fail-closed, writes nothing, engine never imports it; `tests/test_suggest.py`, 17 tests; DEC-0018). What is LEFT is not plumbing: (a) a model worth running on this machine, (b) the approval step in the UI (today a proposal is approved by adding it to the dictionary by hand), (c) a decision about what the prompt may contain. | The structural answer to contextual references — the biggest declared hole. The engine stays the only writer, so determinism is untouched. | large (remaining: medium) |
...
| 21 | 38 | **The web UI in English as well as Italian.** The interface is Italian-only (`web/index.html`, `web/app.js`); the CLI, the code, the tests and the docs are English. The work is not translation but plumbing: a string table with two locales, a switch that remembers the choice, every place that asserts Italian copy (`tests/test_web.py`, `tests/ui_load_check.mjs` — 57 declared ids), the README screenshot, and the README line that says the UI is Italian. The ENGINE is not involved: it is already language-neutral apart from the Italian validators and the `legal` pattern group (see "Italian first, not Italian only"). | The README states it plainly, so an English-speaking reader meets an Italian interface at the first click. | medium |
