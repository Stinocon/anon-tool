...
| 5 | 17 | **Optional local-model detector** (DESIGN §7): a localhost endpoint that *suggests* candidates which a human approves — **the seam AND the approval step exist** (`suggest.py`: loopback-only, fail-closed, writes nothing, engine never imports it, `--api-key`/`--header` for the local server, `reasoning_content` fallback; the UI panel in the Dizionario tab with `--suggest-url/--suggest-model`, off without them; DEC-0018, DEC-0019). What is LEFT is the model: measured with the local MTPLX Qwen3.5-9B, 138 s and 4096 tokens on one sentence with an empty answer (`finish_reason: length` — a reasoning model that thinks until the budget ends). The seam reported it as an error, correctly. Next: a small instruct model, or a larger `--max-tokens`, or point it at any OpenAI-compatible loopback server. | The structural answer to contextual references — the biggest declared hole. The engine stays the only writer, so determinism is untouched. | large (remaining: medium) |
