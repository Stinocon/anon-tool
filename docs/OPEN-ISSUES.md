...
Frozen records are not rewritten. `DEC-0033` said "18 test" (the suite has grown) and named a
script that belongs to a private repository; both drifts were recorded here rather than edited into
a `validated` file, and they are now resolved by `DEC-0034`, which supersedes `DEC-0033` and restates
the same invariant without the count and without the private name. `DEC-0034` also drops the
`verify_anchor` DEC-0033 carried, so its `verify: auto` is exit-code-only (the anchor is optional for
`auto`); that third difference is recorded here because a `validated` record is not edited to declare
it. `DEC-0033` stays frozen, byte for byte.
